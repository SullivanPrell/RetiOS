...
final class MicronPageStore {
  ...
  private func restoreBookmarkedRoot() {
    ...
    do {
      ...
      lastError =
        "Could not reopen the saved page folder (\(error.localizedDescription)). "
    ...
    guard resolved.startAccessingSecurityScopedResource() else {
      lastError =
        "Access to the saved page folder was revoked. "
        + "Using the app's own page storage instead."
      return
    }
    ...
    guard FileManager.default.fileExists(atPath: resolved.path, isDirectory: &isDirectory),
      ...
      lastError =
        "The saved page folder no longer exists. "
        + "Using the app's own page storage instead."
      return
    }
  ...
  private func ensureRootExists() {
    ...
    if fm.fileExists(atPath: rootURL.path, isDirectory: &isDirectory) {
      if !isDirectory.boolValue {
        lastError =
          MicronPageError.rootUnavailable("\(rootURL.path) is a file, not a folder")
          .errorDescription
      }
      return
    }
  ...
  func rename(_ page: MicronPage, to newName: String) throws -> MicronPage {
    ...
    do {
      if sameFile {
        let staging = parent.appending(
          path: ".retios-rename-\(UUID().uuidString)",
    ...
    return try self.page(
      forRelativePath: Self.relativePath(of: destination, under: rootURL) ?? safe,
  ...
  func duplicate(_ page: MicronPage) throws -> MicronPage {
    ...
    reload()
    return try self.page(
      forRelativePath: Self.relativePath(of: destination, under: rootURL)
