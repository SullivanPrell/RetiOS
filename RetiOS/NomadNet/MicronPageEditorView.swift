...
struct MicronPageEditorView: View {
  ...
  private var editor: some View {
    VStack(spacing: 0) {
      ...
      if loadFailed {
        Label(
          "This file could not be read as UTF-8 text and is shown empty. Editing is disabled so it is not overwritten.",
    ...
    HStack(spacing: 10) {
      if diagnostics.isEmpty {
        ...
        Button {
          ...
          Label(
            "\(diagnostics.count) issue\(diagnostics.count == 1 ? "" : "s")",
            systemImage: "exclamationmark.triangle.fill"
          )
          .font(.caption2)
          .foregroundStyle(Color.rnsWarning)
      ...
      rnsSettingsContainer {
        ...
        Section("Preview") {
          ...
          if !link.offendingCharacters.isEmpty {
            Label(
              "Remove \(link.offendingCharacters.map(String.init).joined(separator: " and ")) — Micron ends the link at the first \"]\", and a backtick starts a new part of it.",
              systemImage: "exclamationmark.triangle.fill"
            )
            .font(.caption)
            .foregroundStyle(Color.rnsWarning)
      ...
      rnsSettingsContainer {
        ...
        Section("Preview") {
          ...
          if !field.offendingCharacters.isEmpty {
            Label(
              "Remove \(field.offendingCharacters.map(String.init).joined(separator: " and ")) — Micron ends the field at the first \">\", and \"`\" and \"|\" separate its parts.",
              systemImage: "exclamationmark.triangle.fill"
            )
            .font(.caption)
            .foregroundStyle(Color.rnsWarning)
          ...
          if field.kind.usesValue {
            Text(
              "Write the visible label as plain text after the field — NomadNet's own examples leave the in-field label empty."
            )
            .font(.caption)
            .foregroundStyle(Color.rnsTextSecondary)
          }
