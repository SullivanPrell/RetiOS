...
struct PagesContent: View {
  ...
  var body: some View {
    ...
    .alert("New Page", isPresented: $showNewPageAlert) {
      ...
      Text(
        "Micron pages use the .mu extension. \"index.mu\" is the page a peer gets when they browse your node without naming one."
      )
    }
    .alert(
      "Rename Page",
  ...
  private var emptyState: some View {
    RNSEmptyState(
      ...
      description:
        "Micron pages are the documents a NomadNet node serves. Create one to start writing, or import a .mu file from another node.",
  ...
  var body: some View {
    HStack(spacing: 10) {
      ...
      VStack(alignment: .leading, spacing: 2) {
        ...
        Text(
          "\(page.byteCount.formatted(.byteCount(style: .file))) · \(RNSDate.listTimestamp(page.modified))"
        )
        .font(.caption2)
        .foregroundStyle(Color.rnsTextSecondary)
      }
