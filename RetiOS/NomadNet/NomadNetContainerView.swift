...
struct NomadNetPeersContent: View {
  ...
  private var emptyState: some View {
    RNSEmptyState(
      ...
      description:
        "NomadNet nodes appear here as their announces arrive across the mesh. Tap Browse to load a node's pages."
    )
  }
}
...
private struct NomadNodeRow: View {
  ...
  var body: some View {
    HStack(spacing: 12) {
      ...
      PeerIdentityView(
        name: node.displayName ?? "Unknown Node",
...
struct NomadNetFavoritesContent: View {
  @Environment(\.modelContext) private var context
  @Query(
    filter: #Predicate<NomadNodeEntity> { $0.isFavorite == true },
    ...
    Group {
      if favorites.isEmpty {
        RNSEmptyState(
          ...
          systemImage: "star",
          description:
            "Tap the star next to a node in the Peers list to pin it here for quick access."
        )
      } else {
        List(filtered) { node in
          NomadNodeRow(node: node, onBrowse: onBrowse) {
