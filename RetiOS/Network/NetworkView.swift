  ...
  enum Tab: String, CaseIterable {
    ...
    static var launchSelection: Tab {
      ...
      if let raw = UserDefaults.standard.string(forKey: "startSection"),
        let tab = Tab.allCases.first(where: {
...
private struct PathTableView: View {
  ...
  private var emptyState: some View {
    RNSEmptyState(
      ...
      systemImage: "map",
      description:
        "Reticulum path entries appear here as announces and traffic arrive across the mesh."
    )
  }
}
    ...
    Section("Node Info") {
      if let info = nodeInfo {
        ...
        LabeledContent("Identities known") {
          Text("\(info.identities)").foregroundStyle(.secondary)
        }
