...
struct NetworkVisualizerView: View {
    ...
    GeometryReader { geo in
      ...
      ZStack(alignment: .topLeading) {
        ...
        ForEach(graph.nodes) { node in
          ...
          .frame(
            minWidth: node.kind.hitDiameter,
            minHeight: node.kind.hitDiameter
          )
          .contentShape(Rectangle())
          .onTapGesture { activate(node) }
          .position(
            x: node.normPosition.x * canvas.width,
  ...
  private var graphSummary: String {
    ...
    let ifaces = graph.nodes.filter {
      if case .interface = $0.kind { return true }
      return false
    }.count
    return
      "Network graph: \(ifaces) interface\(ifaces == 1 ? "" : "s"), \(dests) known destination\(dests == 1 ? "" : "s")"
  }
  ...
  private func accessibilityLabel(for node: NetworkGraph.Node) -> String {
    ...
    case .interface(let type):
      ...
      return
        "\(type.label) interface \(node.label), \(reachable) destination\(reachable == 1 ? "" : "s") reachable"
    case .cluster(let type, let band, let count, let active):
      let activePart = active > 0 ? ", \(active) with an active link" : ""
      return
        "\(count) destination\(count == 1 ? "" : "s") \(band.spokenLabel) over \(type.label)\(activePart). Opens the list."
    case .destination(let type, let hops, let active):
      return
        "\(type.label) destination \(node.label), \(hops) hop\(hops == 1 ? "" : "s")\(active ? ", active link" : "")"
    }
  }
  ...
  private func ringsLayer(size: CGSize) -> some View {
    ZStack {
      ForEach(graph.occupiedBands, id: \.self) { band in
        ...
        Ellipse()
          .stroke(
            Color.rnsTextMuted.opacity(0.15),
    ...
    ZStack {
      ForEach(graph.edges) { edge in
        if let from = graph.node(edge.from), let to = graph.node(edge.to) {
          let highlighted =
            selectedNodeID != nil && (selectedNodeID == edge.from || selectedNodeID == edge.to)
          ...
          EdgeShape(
            from: CGPoint(
              x: from.normPosition.x * size.width, y: from.normPosition.y * size.height),
            to: CGPoint(x: to.normPosition.x * size.width, y: to.normPosition.y * size.height)
          )
          .stroke(
            edge.connectionType.color.opacity(baseOpacity),
            style: StrokeStyle(
  ...
  private var dashedSwatch: some View {
    Canvas { context, size in
      ...
      context.stroke(
        path, with: .color(Color.rnsTextMuted),
  ...
  private var emptyState: some View {
    RNSEmptyState(
      ...
      description:
        "The graph fills in as interfaces come online and paths to other destinations are discovered."
    )
  }
...
struct NetworkGraph {
  ...
  init(
    nodes: [Node] = [], edges: [Edge] = [], occupiedBands: [HopBand] = [],
...
extension NetworkGraph {
  ...
  static func build(transport: Transport) -> NetworkGraph {
    ...
    return build(
      interfaces: transport.interfaces.map { iface in
        InterfaceInput(
          name: iface.name,
          type: ConnectionType(
            interfaceTypeName: String(describing: type(of: iface)),
            name: iface.name),
  ...
  static func build(interfaces: [InterfaceInput]) -> NetworkGraph {
    var nodes: [Node] = [
      Node(
        id: meID, label: "Me", fullID: "This node",
    ...
    for (i, iface) in interfaces.enumerated() {
      ...
      for band in HopBand.allCases {
        ...
        let edgeKind: Edge.PathKind =
          (band == .direct && previousID == ifaceID) ? .known : .inferred
  ...
  static var demo: NetworkGraph? {
    ...
    return build(interfaces: [
      .init(
        name: "AutoInterface", type: .auto,
        members: members(14, seed: 0x1234_5678, hops: [1, 1, 1, 2])),
      .init(
        name: "amsterdam.connect.reticulum.network", type: .tcp,
        members: members(203, seed: 0xDEAD_BEEF, hops: [2, 3, 3, 4, 5, 6, 7, 9, 13, 27])),
      .init(
        name: "Simply Equipped Backbone", type: .backbone,
...
private struct NodeBubble: View {
  ...
  var body: some View {
    VStack(spacing: 3) {
      ...
      .shadow(
        color: fillColor.opacity(isSelected ? 0.65 : 0),
...
enum ConnectionType: CaseIterable, Hashable {
  ...
  init(interfaceTypeName: String, name: String) {
    ...
    default:
      ...
      if n.contains("auto") {
        self = .auto
      } else if n.contains("tcp") {
        self = .tcp
      } else if n.contains("udp") {
        self = .udp
      } else if n.contains("rnode") || n.contains("lora") || n.contains("ble") {
        self = .lora
      } else if n.contains("i2p") {
        self = .i2p
      } else if n.contains("local") {
        self = .local
      } else if n.contains("serial") || n.contains("kiss") {
        self = .serial
      } else {
        self = .other
      }
