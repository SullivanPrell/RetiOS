...
struct BLEMeshView: View {
  ...
  private var statusSection: some View {
    Section {
      ...
      if controller.state == .bluetoothUnavailable {
        Text(
          "Bluetooth is unavailable or not authorized. Enable it in Settings to mesh with nearby devices."
        )
  ...
  private var aboutSection: some View {
    Section {
      Text(
        "BLE Mesh lets this device mesh directly with nearby phones running RetiOS over Bluetooth Low Energy — no RNode hardware, router, or internet connection required. Every device that joins extends the reach of the mesh for everyone nearby."
      )
      .font(.footnote)
      .foregroundStyle(.secondary)
    } header: {
      ...
      Text(
        "Uses Bluetooth Low Energy. Range is short — typically tens of meters indoors. Keeping the mesh on increases battery use."
      )
    }
    .rnsRow()
  }
