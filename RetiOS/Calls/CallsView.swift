  ...
  private func incomingCallView(callerHash: Data) -> some View {
    VStack(spacing: 24) {
      ...
      if callerHash.isEmpty {
        ...
        CallContactAction(
          callHash: callerHash, lxstPeerHashes: lxstPeerHashes,
  ...
  private func callingView(hash: Data) -> some View {
    VStack(spacing: 24) {
      ...
      Text(hash.hexString.truncatedHash)
        ...
        .lineLimit(1)
      CallContactAction(
        callHash: hash, lxstPeerHashes: lxstPeerHashes,
        liveIdentity: calls.activeCallRemoteIdentity)
        ...
        VStack(spacing: 8) {
          Button(action: { calls.toggleMute() }) {
            Image(systemName: calls.isMuted ? "mic.slash.fill" : "mic.fill")
              ...
              .background(
                calls.isMuted
                  ? AnyShapeStyle(Color.rnsWarning)
...
private enum CallPeerResolver {
  ...
  static func lxmfDeliveryHex(
    forCallHash hash: Data,
    lxstPeerHashes: [String] = [],
    liveIdentity: Identity? = nil
  ) -> String? {
    ...
    // an inbound call from a peer that has not announced this session,
    ...
    // address is warranted even though whether they run LXMF at all is not yet
    // knowable.
    if let liveIdentity {
    ...
    // Case A—`hash` is a destination hash heard in an announce (an LXST
    ...
    // Case B—the caller is one of the LXST peers heard announcing.
    ...
    // so the delivery address is computable from the identity hash alone—the
    // same bytes the `Destination` initialiser would produce given
...
private func saveCallPeerAsContact(lxmfHex: String, in context: ModelContext) {
  ...
  if let existing = try? context.fetch(descriptor).first {
    ...
    // was still derived from their real Identity, so this is their address—it
    // just has not been heard announced yet.
...
private struct CallContactAction: View {
  ...
  ///     a peer that has not announced this session resolves to
  ...
  init(callHash: Data, lxstPeerHashes: [String], liveIdentity: Identity? = nil) {
    let hex = CallPeerResolver.lxmfDeliveryHex(
      forCallHash: callHash,
    ...
    let target = hex ?? ""
    _matches = Query(
      filter: #Predicate<PeerEntity> { $0.destinationHash == target },
...
private struct CallsPeersContent: View {
  ...
  private var resolvedPeers: [ResolvedCallPeer] {
    let byHash = Dictionary(
      knownPeers.map { ($0.destinationHash, $0) },
  ...
  var body: some View {
    if calls.lxstPeers.isEmpty {
      RNSEmptyState(
        ...
        description:
          "Peers that have announced their LXST call address will appear here. Enable LXST announcing in Settings so others can call you too."
      )
    } else {
  ...
  private func contactButton(for entry: ResolvedCallPeer) -> some View {
    if let lxmfHex = entry.lxmfHex {
      Button {
        ...
        Label(
          entry.isContact ? "Remove Contact" : "Add Contact",
          systemImage: entry.isContact
