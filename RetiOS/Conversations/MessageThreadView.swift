    ...
    HStack(spacing: 8) {
      ...
      TextField("Message", text: $draft, axis: .vertical)
        ...
        .onChange(of: draft) { _, _ in
          if isResettingDraft { isResettingDraft = false } else { onDraftChanged() }
        }
...
private struct MessageBubble: View {
  ...
  var body: some View {
    ...
    return HStack {
      ...
      // this view is widest.
      .frame(
        maxWidth: RNSLayout.bubbleWidth,
  ...
  var body: some View {
    VStack(alignment: outbound ? .trailing : .leading, spacing: 6) {
      if let image = attachments.image, let rendered = platformImage(image.data) {
        ...
        chip(
          icon: "photo", title: "Image", detail: "\(image.type) · \(byteLabel(image.data.count))")
      }
