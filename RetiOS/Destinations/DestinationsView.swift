...
struct DestinationsView: View {
  ...
  private var filtered: [PeerEntity] {
    ...
    return peers.filter {
      ($0.displayName?.lowercased().contains(q) ?? false) || $0.destinationHash.contains(q)
    }
  }
  ...
  var body: some View {
    NavigationStack {
              ...
              Button {
                if let hashData = Data(hexString: peer.destinationHash) {
                  if calls.hasLXSTCallPath(for: hashData) {
                    ...
                    lxstError =
                      "\(peer.label) has not announced their LXST call destination. Ask them to enable LXST on their Reticulum node so it can advertise its call address."
                  }
                }
              } label: {
      ...
      .confirmationDialog(
        ...
        Text(
          "“\(peer.label)” will be removed along with any nickname you set. They'll reappear if they announce again."
        )
      }
    }
  }
  ...
  private var peersEmptyState: some View {
    RNSEmptyState(
      ...
      description:
        "Peers appear automatically as their LXMF announcements arrive across the mesh. Make sure the stack is running and at least one interface is active."
    )
  }
}
...
private struct PeerRow: View {
  ...
  var body: some View {
    ...
    PeerIdentityView(
      name: peer.displayName ?? "Unknown Peer",
