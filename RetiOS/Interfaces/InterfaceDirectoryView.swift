...
struct InterfaceDirectorySheet: View {
    ...
    NavigationStack {
      ...
      List {
        ...
        if !filtered.isEmpty {
          Section {
            ...
            Text(
              "Entries are reported online by community members. Connecting routes your traffic through their node — only add gateways you trust."
            )
          }
        }
      }
      ...
      .overlay {
        if entries.isEmpty && loadError == nil {
          ...
          if search.isEmpty {
            RNSEmptyState(
              title: "No Interfaces Online", systemImage: "network.slash",
  ...
  private func add(_ entry: InterfaceDirectory.Entry) {
    guard let kind = entry.savedKind, let port = entry.port, let portNum = UInt16(exactly: port)
    else { return }
    addError = nil
    do {
