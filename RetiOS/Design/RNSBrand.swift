...
struct RNSSectionPicker<T: Hashable>: View {
  ...
  init(_ tabs: [(String, T)], selection: Binding<T>) {
    self.tabs = tabs.enumerated().map {
      Tab(id: $0.offset, label: $0.element.0, value: $0.element.1)
    }
...
extension View {
  ...
  func rnsInlineSearch(
    text: Binding<String>,
    prompt: LocalizedStringKey = "Name or hash"
  ) -> some View {
    VStack(spacing: 0) {
...
public struct AddressActionRow: View {
  let label: String
  let fullHex: String
  let isRunning: Bool
  let onAnnounce: () -> Void
