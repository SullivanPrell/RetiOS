...
final class LXMFMessageIngest {
  ...
  private func flush() {
    ...
    for item in batch where !known.contains(item.messageHash) {
  ...
  private func notify(_ inserted: [Incoming], context: ModelContext) {
    ...
    if inbound.count > burstThreshold {
      let senders = Set(
        inbound.map { names[$0.conversationHash] ?? "\($0.conversationHash.prefix(8))…" })
      ...
      banners = inbound.map { item in
        Banner(
          senderName: names[item.conversationHash] ?? "\(item.conversationHash.prefix(8))…",
