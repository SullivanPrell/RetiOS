...
struct RootView: View {
  ...
  var body: some View {
    ...
    .frame(minWidth: 820, minHeight: 520)
      ...
      .modifier(
        OnboardingPresenter(isPresented: $showOnboarding) {
...
extension View {
  @ViewBuilder
  fileprivate func suppressTabBarMinimize() -> some View {
    #if os(iOS)
...
private struct OnboardingView: View {
  ...
  private var welcomeStep: some View {
    VStack(spacing: 18) {
      ...
      Text(
        "A complete client for the Reticulum mesh — messaging, calls, and pages, with no servers or accounts."
      )
      .font(.body)
      .foregroundStyle(.secondary)
      .multilineTextAlignment(.center)
      VStack(alignment: .leading, spacing: 6) {
        ...
        Text(
          "Included in your announces so peers see a name instead of a raw address. You can change it anytime in Settings."
        )
        .font(.footnote)
        .foregroundStyle(.secondary)
      }
  ...
  private var addressStep: some View {
    VStack(spacing: 16) {
      ...
      Text("Your address")
        .font(.title.bold())
      Text(
        "Your identity hash is your address on the network. Share it — or its QR code — so others can message or call you."
      )
      .font(.body)
      .foregroundStyle(.secondary)
      .multilineTextAlignment(.center)
    ...
    .task(id: stack.identity?.hexHash) {
      guard let hex = stack.identity?.hexHash else {
        qrImage = nil
        return
      }
...
private struct OnboardingConnectStep: View {
  ...
  var body: some View {
    VStack(spacing: 14) {
      Text("Connect to the mesh")
        .font(.title.bold())
      Text(
        "Add a community gateway to reach the wider network over the internet. Optional — you can mesh locally or add interfaces later in Settings."
      )
      .font(.body)
      .foregroundStyle(.secondary)
      .multilineTextAlignment(.center)
      ...
      if entries.isEmpty && !loadFailed {
        ...
        Label(
          "No gateways available right now. You can add one later in Settings ▸ Interfaces.",
    ...
    .task {
      ...
      do { entries = try await InterfaceDirectory.fetchOnline() } catch { loadFailed = true }
    }
  }
  ...
  private func gatewayRow(_ entry: InterfaceDirectory.Entry) -> some View {
    let isAdded =
      added.contains(entry.name) || stack.savedInterfaces.contains { $0.name == entry.name }
    HStack(spacing: 12) {
  ...
  private func add(_ entry: InterfaceDirectory.Entry) {
    guard let kind = entry.savedKind,
      let port = entry.port,
      let portNum = UInt16(exactly: port)
    else { return }
