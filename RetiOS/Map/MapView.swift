...
struct MapView: View {
  ...
  private var permissionMessage: String {
    ...
    case .notDetermined:
      return
        "RetiOS can show your position on the map and, later, share it with the mesh. Grant location access to get started."
    default:
      return
        "Location access is currently denied. Enable it in Settings to see your position on the map."
    }
  }
  ...
  private var sourceStatusRow: some View {
    ...
    case .device:
      statusLine(
        ok: locationManager.location != nil,
  ...
  var body: some View {
    NavigationStack {
      List {
        Section {
          Label(
            "Recently viewed areas stay available offline", systemImage: "checkmark.circle.fill"
          )
          .foregroundStyle(Color.rnsSuccess)
          Text(
            "Apple Maps tiles you've already viewed are cached automatically, so areas you've explored remain visible without a connection."
          )
          .font(.caption)
          .foregroundStyle(.secondary)
        } header: {
        ...
        Section {
          Text(
            "Downloadable offline region packs — so you can pre-fetch an area before heading off-grid — are planned for a future update, alongside RNode-sourced GPS and shared mesh positions."
          )
          .font(.caption)
          .foregroundStyle(.secondary)
        } header: {
