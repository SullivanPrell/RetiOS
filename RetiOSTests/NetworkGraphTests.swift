...
final class NetworkGraphTests: XCTestCase {
  ...
  private func members(_ count: Int, hops: [UInt8], active: Int = 0) -> [NetworkGraph.Member] {
    (0..<count).map { i in
      NetworkGraph.Member(
        hashHex: String(format: "%032x", i + 1),
  ...
  private func iface(
    ...
    _ members: [NetworkGraph.Member]
  ) -> NetworkGraph.InterfaceInput {
  ...
  func testSmallBandsRenderIndividualDestinations() {
    ...
    let dests = graph.nodes.filter {
      if case .destination = $0.kind { return true }
      return false
    }
    let clusters = graph.nodes.filter {
      if case .cluster = $0.kind { return true }
      return false
    }
  ...
  func testLargeBandsCollapseIntoOneCluster() {
    ...
    XCTAssertEqual(
      clusters.first?.members.count, 200,
  ...
  func testAllNodesLandInsideTheCanvas() {
    ...
    for node in graph.nodes {
      XCTAssertTrue(
        (0.0...1.0).contains(node.normPosition.x), "\(node.id) x=\(node.normPosition.x)")
      XCTAssertTrue(
        (0.0...1.0).contains(node.normPosition.y), "\(node.id) y=\(node.normPosition.y)")
    }
  }
